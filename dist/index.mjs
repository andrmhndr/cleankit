...
import React5, { createContext as createContext2, useCallback, useState } from "react";
...
var Button = newStyled.button`
  ...
  border: none;
  border-radius: 6px;
  background-color: ${({ variant }) => variant === "ok" ? "#0070f3" : "#aaa"};
  color: white;
  cursor: pointer;
...
var DialogProvider = ({ children }) => {
  ...
  const openDialog = useCallback((dialog) => {
    ...
    return new Promise((resolve) => {
      setDialogs((prev2) => [...prev2, { ...dialog, id, resolve }]);
    });
  }, []);
  const closeDialog = useCallback((id) => {
    if (!id) {
      setDialogs((prev2) => prev2.slice(0, -1));
      return;
    }
    setDialogs((prev2) => prev2.filter((d) => d.id !== id));
  }, []);
  ...
  return /* @__PURE__ */ React5.createElement(DialogContext.Provider, { value: { openDialog, closeDialog, closeAllDialog } }, children, dialogs.map((dialog) => /* @__PURE__ */ React5.createElement(
    Overlay,
    {
      key: dialog.id,
      onClick: () => resolveDialog(dialog.id, null)
    },
    /* @__PURE__ */ React5.createElement(DialogBox, { onClick: (e) => e.stopPropagation() }, typeof dialog.content === "function" ? dialog.content(
      (result) => resolveDialog(dialog.id, result)
    ) : dialog.content ?? /* @__PURE__ */ React5.createElement(ButtonGroup, null, /* @__PURE__ */ React5.createElement(
      Button,
      {
...
var useDialog = () => {
  const context = useContext5(DialogContext);
  if (!context) {
...
// src/query-params/next/useQueryParams.hook.next.ts
import { useRef as useRef2 } from "react";
import { useRouter } from "next/router";
var useQueryParamsNext = () => {
  const router = useRouter();
  const debounceMap = useRef2({});
  const getAll = () => {
...
// src/query-params/react/useQueryParams.hook.react.ts
import { useRef as useRef3 } from "react";
import { useLocation, useNavigate } from "react-router-dom";
var useQueryParamsReact = () => {
  const location = useLocation();
  const navigate = useNavigate();
  const debounceMap = useRef3({});
